...
- **自动接入**：机器上装了哪些 Agent CLI（`HAPI_AGENTS=auto` 按 PATH 探测；容器部署写
  显式清单），就自动全部建成 AI 用户出现在联系人里；管理员可在「AI 管理」页随手关掉
  某个（不会被强行拉回）、也可手动打开探测漏网的；机器离线自动隐身、恢复自动回来；
  可改显示名（不许带空格，提及按整名匹配）；「测试连通性」一键验三层。
...
```bash
npm run test          # 后端 940 条 + 前端 934 条（约 1 分钟）
...
| 层次 | 用例 | 覆盖内容 |
| --- | --- | --- |
| 后端 `server/test` | 940（59 文件） | 登录 / 权限 / 限流 / 会话与群管理 / 消息与 @ 机制 / 已读回执 / 表情回应 / 引用回复 / 搜索 / 附件安全（嗅探、鉴权、Range）/ 对象存储 / Web Push（加密、订阅、该不该推）/ 安全默认值与日志脱敏 |
| 前端 `web/src/**/*.test.*` | 934（78 文件） | Markdown 渲染与 XSS 转义 / 乐观发送与合并排序 / 输入框与 @ 提及 / 通知状态机 / 推送订阅与 sw.js 源码约束 / 主题跟随系统与手动记忆 / 各组件交互 |
