...
```bash
npm run test          # 后端 938 条 + 前端 929 条（约 1 分钟）
...
| 层次 | 用例 | 覆盖内容 |
| --- | --- | --- |
| 后端 `server/test` | 938（59 文件） | 登录 / 权限 / 限流 / 会话与群管理 / 消息与 @ 机制 / 已读回执 / 表情回应 / 引用回复 / 搜索 / 附件安全（嗅探、鉴权、Range）/ 对象存储 / Web Push（加密、订阅、该不该推）/ 安全默认值与日志脱敏 |
