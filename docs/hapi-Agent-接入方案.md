...
最后更新：2026-08-31 · 状态：**PR1 / PR2 / PR3 全部完成合入**——@ Agent 已能真实对话，剩上线（Sophie 侧准备 + 切配置部署）
...
| D13 | **消息原样转发**（2026-08-31，替代早期「拼上下文」想法） | 上下文由 hapi 会话自身在底层携带、由本地 Agent CLI 管理——我们只把这条消息发出去（参照 HapiKmp 手机客户端：请求体就是 {text, localId}）。会话模型因此改为**每个「Agent × IM 会话」一条 hapi 会话**（§C.3'）。唯一进文本的是群聊署名「张三：」（接口没有发送者字段）；私聊原文直达，**第一条也零注入**——人设/规矩在工作目录的 CLAUDE.md / AGENTS.md（启用 Agent 时自动铺设，已有不覆盖；线上容器摸不到宿主目录，铺文件属部署清单） |
