...
| D15 | **执行过程落库可回看**（2026-09-01，用户实测拍板） | Agent 干活的中间过程（说明文字、工具动作）此前全被丢弃——只贴最终回复，等待期间没有任何反馈，事后也无从回看（Codex 画图实测：4 段中间文字 + 8 次工具调用全部不可见）。修法：独立表 `hapi_turn_steps` 按步落库（turn_id 归组，回复贴出后挂 `reply_message_id`），**中间文字存全文、工具动作存一句人话标签**（优先工具自带描述，其次剥壳后的命令/路径）；工具结果与推理独白不存（噪音）；同一调用的多条状态更新按 callId 只记一次；hapi 内部杂务（改会话标题）不记；步数封顶 `HAPI_STEPS_MAX`（默认 200）。展示两态同源、**平铺流式**（D15'，2026-09-01 二次拍板）：进行中气泡随 `ai-progress` 一步步长出来——中间文字直接是正文段落、工具动作只留扳手小徽章（连续几步合并成一个 ×N，命令明细收进悬停提示——三次拍板：过程里图标即可，字是噪音）、尾部保留跳动的点；收工时气泡内画一条分割线，最终结论排在线下最底部；历史消息把过程**整份内嵌**在消息里（`progress` 数组），一进来就全可见，不设点开才拉的二段式（原「执行过程 · N 步」折叠行与 steps 查询接口随之取消）。失败文案（暂不可用/超时）同样挂过程——出事时一看就知道卡在哪步。私聊群聊同一套。实现见 `server/src/hapi/steps.js` |

| D16 | **附件双向摆渡**（2026-09-01，用户实测拍板） | 此前与 Agent 之间只传纯文字：用户发的图对它是「[图片]」占位，它交付的图我们不去取——两个方向都断在我们这层，hub 其实全支持（照 HapiKmp 客户端逐口验证）。**用户→Agent**：正文引用的站内附件递话前从我们的存储读出、经 `POST /api/sessions/:id/upload`（JSON+Base64）传进会话，正文链接换成「已放到 <runner 路径>」（Agent 用自己的工具就能读），消息同时带附件元数据；群聊补课批次里的行同样摆渡。传不了的（过大/超个数/失败）留占位注明原因。**Agent→用户**：回合里 display_image 交付的图片（generated-image 事件）收工后经 `GET /api/sessions/:id/generated-images/:imageId` 下载、存成我们的附件（挂下载白名单）、以 Agent 名义作为图片消息贴出（跟在文字回复后、不重复引用；超时了也照贴——真实产物不能扔）。上限：单文件 `HAPI_ATTACH_MAX_MB`（默认 15）、单回合双向各 `HAPI_ATTACH_MAX_PER_TURN`（默认 5）。这类文件操作由会话进程代执行，仅活跃会话可用——递话前本就要判活，顺路。实现见 `server/src/hapi/files.js` |
