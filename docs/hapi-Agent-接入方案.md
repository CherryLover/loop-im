...
| D2 | **配置页勾选，自动建用户** | AI 配置页从 hapi 拉「本机可用 Agent 列表」，管理员勾选启用；系统对启用的 Agent 自动创建/启用用户，取消勾选则自动停用 |
...
| 阶段 | 内容 | 出口条件 |
| --- | --- | --- |
| PR1 | hub 客户端（auth/判活/spawn/messages/SSE）+ 配置 + Agent→用户映射与联动；**不动 Aria** | mock 下全绿；本地真 hub 手工验通 |
| PR2 | 消息流转全链路：@ 触发、队列、上下文前缀、回复贴回、D6 文案、超时 | 本地 hub 群聊/私聊真跑通 |
| PR3 | Aria 退役（§F 全部）+ 文档与测试全量对齐 | 全套测试绿；README/测试用例集改写完 |
